...
struct GpioPort {
  ...
  IGB_FAST_INLINE void setAlternateFunc(uint32_t pin_bit, GpioAf af) {
    ...
    if (lsb) {
      IGB_MODIFY_REG(p_gpio->AFR[0], ((((lsb * lsb) * lsb) * lsb) * IGB_BIT_MASK(4, 0)), ((((lsb * lsb) * lsb) * lsb) * as<uint32_t>(af)));
    }
    if (msb) {
      IGB_MODIFY_REG(p_gpio->AFR[1], ((((msb * msb) * msb) * msb) * IGB_BIT_MASK(4, 0)), ((((msb * msb) * msb) * msb) * as<uint32_t>(af)));
    }
  }
